...
export function OrganizationTabs({}: Props) {
  ...
  const [currentTab, setCurrentTab] = useState<string>(
    (tab as string) || organizationTabs[0].key,
  );

  useEffect(() => {
    if (organizationTabs.some((tabItem) => tabItem.key === (tab as string))) {
  ...
  return (
    <Tabs
      ...
      onValueChange={(value) => {
        setCurrentTab(value);
        router.push(`/dashboard/${selectedOrganization?.id}?tab=${value}`);
      }}
    >
